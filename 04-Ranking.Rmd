...
\textbf{RQ: } How can we rank algorithm different optimization algorithms given a budget of 10,000 evaluations per dimension in noisy benchmarks?
...
The Stan model is specified in the file: `'./stanmodels/rankingmodel.stan'`

```{r}
print_stan_code('./stanmodels/rankingmodel.stan')
```
...
saveRDS(standata, file = "./data/rankingmodel-data.RDS")
```
...
```{r echo=T, eval=F}
standata<-readRDS("./data/rankingmodel-data.RDS")
ranking.fit <- stan(file = './stanmodels/rankingmodel.stan',
                     data=standata,
                     chains = 4,
                     warmup = 200,
                     iter = 3000)
saveRDS(ranking.fit, file = "./data/ranking-fit.RDS")
```

```{r echo=F, include=F, eval=T}
ranking.fit <-readRDS("./data/ranking-fit.RDS")
```

## Diagnosis

```{r}
rstan::traceplot(ranking.fit)
```
...
hpdi_algorithm <- hpdi %>% 
      dplyr::filter(str_detect(Parameter, "alpha\\[")) %>%
...
alpha <- as_tibble(posterior$alpha)
colnames(alpha) <- algorithms
#sampling from the posterior
s <- dplyr::sample_n(alpha, size = 1000, replace=T)
s <- dplyr::mutate(s, rown = row_number())
wide_s <- tidyr::pivot_longer(s, cols=all_of(algorithms), names_to = "Algorithm", values_to = "Alpha")
rank_df <- wide_s %>% 
  dplyr::group_by(rown) %>% 
  dplyr::mutate(Rank = rank(-Alpha, ties.method = 'random')) %>% 
  dplyr::ungroup() %>% 
  dplyr::select(-Alpha) %>% 
