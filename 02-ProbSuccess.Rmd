...
```{r echo=T}
dataset<-select(dataset, Algorithm, CostFunction, SD, MaxFevalPerDimensions, simNumber, SolveAt1, "SolveAt1e-1","SolveAt1e-3","SolveAt1e-6", OptimizationSuccessful)
```
...
Then we restrict to the algorithms, them to the slopes, then to the other parameters
```{r}
...
t<-create_table_model(probsuccess.fit, c(a_alg, b_noise, 's'), rename_pars)
